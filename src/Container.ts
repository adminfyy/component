...
export const StickyContainer = forwardRef<HTMLDivElement,IStickyContainerProps>((props, ref) => {
    const subscribers = useRef<Function[]>([]);
    const framePending = useRef(false);
    const divRef = useEnsuredForwardedRef<HTMLDivElement>(ref as React.MutableRefObject<HTMLDivElement>);
    ...
    const notifySubscribers = useCallback(evt => {
        ...
        rafHandle.current = raf(() => {
            ...
            const { top, bottom } = divRef.current?.getBoundingClientRect() || { top: 0, bottom: 0 };
            const event = {
                distanceFromTop: top,
                distanceFromBottom: bottom,
                eventSource: currentTarget === window ? document.body : divRef.current
            }
    ...
    const delegates: Record<string, Function> = useMemo(() => events.reduce((d, event) => {
        d[`on${event}`] = notifySubscribers
        return d
    },{} as Record<string, Function>),[notifySubscribers])
    ...
    const unsubscribe = useCallback((handler) => {
        subscribers.current = (subscribers.current.filter(current => current !== handler))
    }, [subscribers])

    return <StickContext.Provider value={{
        subscribe,
        unsubscribe,
